...
http://localhost/webapp_p2p/Genes.php
*/
?>
<?php
/*
...
<input type="text"  name="gene_id" placeholder="16499" class="textbox"
	<?php
		if(!empty($connection))
		{			
			echo 'value="' . $gene_id .'"'; 
...
if (isset($_POST['submit1']) && empty($_POST['gene_id'])) 
	...
	{	
			...
			if ($NumOfRows == 0)
			{
				echo "No Gene records with the provided Gene ID <u>$gene_id</u>.";
			} else
			{
				echo '<table class="table1">';
				echo '<caption>
					Gene details if there are associated protein(s) ';
				echo "</caption>";
				echo "<thead>
						<tr>";
